...
public class RoomService {
    ...
    // 방 생성
    @Transactional
    public void createRoom(RequestCreateRoomDTO requestDTO, UserEntity creator) {
    ...
    // 방 수정
    @Transactional
    public void updateRoom(String roomName, String roomDescription, Long roomId) {
        // 방 수정 로직
        ...
        room.update(roomName, roomDescription);
    ...
    public void deleteRoom(Long roomId) {
        ...
        // 방이 수정/생성된 지 10분이 지나지 않았다면 삭제 불가
        if (room.getCreateDate().plusMinutes(10).isAfter(java.time.LocalDateTime.now())) {
            throw new IllegalArgumentException("방 생성 후 10분 이내에는 삭제할 수 없습니다.");
        }
